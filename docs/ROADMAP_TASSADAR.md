...
| `PTAS-504` | implemented | Added the machine-readable learned horizon policy at `fixtures/tassadar/reports/tassadar_learned_horizon_policy_report.json`. It now anchors `learned_article_class` to the exact committed Hungarian-10x10 benchmark artifact while still keeping broader learned long-horizon widening explicit. |
| `PTAS-505` | implemented | Added the learned article-closure audit at `docs/audits/2026-03-17-tassadar-learned-article-closure-audit.md`. It is explicitly subordinate to `fixtures/tassadar/reports/tassadar_acceptance_report.json` and now records the green fixed-corpus learned article closure. |
...
| `PTAS-604` | implemented | `psionic-serve` now has the specialized `psionic.article_hybrid_workflow` contract above `psionic.planner_executor_route`, with canonical article-case resolution, planner-owned routing receipts, preserved benchmark identity, preserved proof identity on delegated success, and typed planner fallback/refusal truth on unsupported routes. The committed artifact `fixtures/tassadar/reports/tassadar_article_hybrid_workflow_artifact.json`, generated by `cargo run -p psionic-serve --example tassadar_article_hybrid_workflow_artifact`, records one delegated exact workflow, one typed planner fallback when runtime decode fallback is disallowed, and one typed over-budget refusal. |
| `PTAS-605` | implemented | Added the final closeout audit at `docs/audits/2026-03-17-tassadar-article-parity-closeout-audit.md`. It is explicitly subordinate to `fixtures/tassadar/reports/tassadar_acceptance_report.json`, names the canonical checker and required artifact roots, and keeps the conclusion binary: today it stays red because `compiled_article_class` and bounded fast-path truth are green but `learned_article_class` and `article_closure` remain red. |
