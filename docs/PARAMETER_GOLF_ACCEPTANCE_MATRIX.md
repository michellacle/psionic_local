...
| `distributed-throughput-closure` | `partial` | Psionic has one explicit `8xH100` execution path with topology, timing, memory, and artifact receipts that support the declared challenge bar. | `psionic-eval` plus `psionic-train` now ship a dedicated Parameter Golf benchmark package and receipt lane covering local-reference score, wallclock, memory, artifact-size, and train or eval bundle-root review artifacts, but there is still no real distributed `8xH100` trainer lane or widened CUDA training surface behind those receipts yet. | `PGOLF-301` / `#169`, `PGOLF-302` / `#170`, `PGOLF-303` / `#171` | Treat the landed receipt lane as bookkeeping substrate only until the actual distributed path exists. |
