...
| --- | --- | --- | --- | --- | --- |
| `challenge-oracle-parity` | `planned` | Psionic reproduces the current FineWeb shard contract, fixed validation split, exact `val_loss`, and exact `val_bpb` accounting against frozen challenge fixtures. | No Parameter Golf data contract, SentencePiece byte-accounting oracle, or parity fixtures are landed in `psionic-data` or `psionic-eval` yet. | `PGOLF-101` / `#163`, `PGOLF-102` / `#164`, `PGOLF-103` / `#165` | Do not claim comparable Parameter Golf scores until this category is green. |
