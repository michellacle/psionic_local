...
| `PGOLF-616` / [#260](https://github.com/OpenAgentsInc/psionic/issues/260) | open (2026-03-19) | `Psionic Parameter Golf: add RMSNorm autodiff and reference-eval support` | Widen the repo from forward CUDA-only RMSNorm execution to an honest train-visible RMSNorm graph surface by adding autodiff and reference-eval closure first, before the CUDA backward path lands. |
...
- `PGOLF-616` -> add RMSNorm autodiff and reference-eval closure so the train-visible graph exists above the backend
