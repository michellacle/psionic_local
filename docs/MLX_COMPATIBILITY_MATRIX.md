...
| `mlx_package_ecosystem` | `convertible` | Bounded local MLX-style text, catalog, text-serving, multimodal, audio, recipe, and workflow packages now exist in `psionic-mlx-lm`, `psionic-mlx-catalog`, `psionic-mlx-serve`, `psionic-mlx-vlm`, `psionic-mlx-audio`, `psionic-mlx-recipes`, and `psionic-mlx-workflows`, but benchmark packages and final ecosystem examples/guides remain later work. | `PMLX-708` through `PMLX-709` | The first local text, catalog, text-serving, multimodal, audio, recipe, and workflow packages are real, but they do not imply benchmark-package completion or the full examples/guides layer. |
