...
| `mlx_package_ecosystem` | `convertible` | Bounded local MLX-style text, catalog, text-serving, multimodal, audio, and recipe packages now exist in `psionic-mlx-lm`, `psionic-mlx-catalog`, `psionic-mlx-serve`, `psionic-mlx-vlm`, `psionic-mlx-audio`, and `psionic-mlx-recipes`, but synthetic/publish workflows and benchmark packages remain later ecosystem work. | `PMLX-707` through `PMLX-709` | The first local text, catalog, text-serving, multimodal, audio, and recipe packages are real, but they do not imply synthetic/publish workflow closure or benchmark-package completion. |
