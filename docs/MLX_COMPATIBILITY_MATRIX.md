...
| `mlx_package_ecosystem` | `convertible` | Bounded local MLX-style text, catalog, and text-serving packages now exist in `psionic-mlx-lm`, `psionic-mlx-catalog`, and `psionic-mlx-serve`, but multimodal/audio packages, recipe layers, synthetic/publish workflows, and benchmark packages remain later ecosystem work. | `PMLX-704` through `PMLX-709` | The first local text, catalog, and text-serving packages are real, but they do not imply multimodal/audio closure, training recipes, synthetic/publish workflows, or benchmark-package completion. |
