...
bash scripts/check-tassadar-post-article-plugin-authority-promotion-publication-and-trust-tier-gate.sh
...
jq -e '
  ...
  and ((.supporting_material_rows | length) == 15)
...
jq -e '
  ...
  and .closeout_status == "operator_green_served_suppressed"
  and .supporting_material_row_count == 15
